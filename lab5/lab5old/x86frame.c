...
F_access F_allocLocal(F_frame frame, bool escape) {
	...
	if (escape) {	
		...
		return F_inFrame(frame->s_size);
	}
	// alloc in register
	else {			
		return F_inReg(Temp_newtemp());
	}
}
	...
	while (escapes) {
		...
		if (escapes->head) {
			if (pnum >= 6) {
				...
				ftail->tail = F_AccessList(F_inFrame(s_size), NULL);
				vtail = vtail->tail;
				ftail = ftail->tail;
			}
			else {
				ftail->tail = F_AccessList(F_inFrame(formal_off), NULL);
				ftail = ftail->tail;
				formal_off += wordsize;
			}
		...
		// alloc in register
			...
			if (pnum >= 6){
				...
				ftail->tail = F_AccessList(F_inReg(temp), NULL);
				vtail = vtail->tail;
				ftail = ftail->tail;
			}
		}
