...
Tr_exp Tr_nilExp()
{
	return Tr_Nx(T_Exp(T_Const(0))); // TODO: modify later
}
...
Tr_exp Tr_callExp(Temp_label label, Tr_level callee, Tr_level caller, Tr_expList args)
{
	...
	if (Temp_labelIn(E_libraryFuncLabels(), label)) {
...
Tr_exp Tr_recordExp(int n, Tr_expList fields)
{
	...
	for (int i = 1; l; l=l->tail, i++) {
		seq = T_Seq(seq, T_Move(T_Mem(T_Binop(T_plus, T_Temp(r), T_Binop(T_mul, T_Const(i), T_Const(F_wordSize)))), unEx(l->head))); //TODO:check later
	}
...
Tr_exp Tr_forExp(Tr_access i, Tr_exp lo, Tr_exp hi, Tr_exp body, Temp_label done)
{
	Temp_label testt = Temp_newlabel();
	Temp_label bodyy = Temp_newlabel();
	T_exp iexp = F_Exp(i->access, T_Temp(F_FP())); // TODO: check if hi == LIMITED
	/*
	 * 		i = lo
	 * test:
...
Tr_exp Tr_noExp()
{
	return Tr_Nx(T_Exp(T_Const(0))); //TODO:check later
}
